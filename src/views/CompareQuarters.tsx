...
export default function CompareQuarters({ history }: { history: HistoryItem[] }) {
  ...
  return (
    <div className="page">
      ...
      <div style={{ marginBottom: 16 }}>
        <div>Long-term Performance Report</div>
        <div style={{ border: "1px dashed #cbd5e1", padding: 16 }}>
          {history.length ? (
            <div>
              <div>Last Period: {history[history.length - 1].periodLabel}</div>
              <div>Cumulative Value: {history.reduce((acc, h) => acc + h.value, 0).toFixed(2)}</div>
            </div>
          ) : (
            <div>No data</div>
          )}
